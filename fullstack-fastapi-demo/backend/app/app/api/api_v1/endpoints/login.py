...
async def enable_totp_authentication(
    *,
    ...
    if current_user.hashed_password:
        password_str = data_in.password.get_secret_value() if data_in.password else None
        user = await crud.user.authenticate(db, email=current_user.email, password=password_str)
        if not password_str or not user:
